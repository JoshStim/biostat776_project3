...
ts_lyrics.mod.words %>%
  ...
  filter(country == "US") %>%
  ggplot(aes(x = released, y = mean_sentiment, color = Album)) +
  geom_point(aes(size = sales)) +
  geom_segment(aes(x = released, y = 0, xend = released, yend = mean_sentiment)) +
  geom_hline(yintercept = 0) +
  theme_minimal()
